...
var GRAPH_URL = 'data/junction_graph.json?v=2026-07-26';
var CHANGES_URL = 'data/junction_changes.json?v=2026-07-26';
...
function JunctionApp() {
  ...
  var _useState5 = useState({ lat: 31.756462, lng: 35.214686 });
  ...
  var _useState6 = useState({ lat: 31.759527, lng: 35.215198 });
  ...
  var debug = _useState132[0];
  var setDebug = _useState132[1];

  var _useState14 = useState('final');

  var _useState142 = _slicedToArray(_useState14, 2);

  var detailState = _useState142[0];
  var setDetailState = _useState142[1];
  ...
  var draw = _useState152[0];
  var setDraw = _useState152[1];

  var _useState16 = useState([]);

  var _useState162 = _slicedToArray(_useState16, 2);

  var drawPts = _useState162[0];
  var setDrawPts = _useState162[1];
  ...
  var freeText = _useState172[0];
  var setFreeText = _useState172[1];

  var _useState18 = useState([]);

  var _useState182 = _slicedToArray(_useState18, 2);

  var freeMuts = _useState182[0];
  var setFreeMuts = _useState182[1];

  var _useState19 = useState('');

  var _useState192 = _slicedToArray(_useState19, 2);

  var freeMsg = _useState192[0];
  var setFreeMsg = _useState192[1];
  ...
  var routeToday = useMemo(function () {
    return !todayWG ? null : mode === 'transit' ? routeWalk(todayWG, nearestNode(todayWG, A, 'walk'), nearestNode(todayWG, B, 'walk')) : route(todayWG, mode, A, B);
  }, [todayWG, mode, A, B, status]);
  var routeFinal = useMemo(function () {
    return !finalWG ? null : mode === 'transit' ? routeTransit(lrtRef.current, A, B) : route(finalWG, mode, A, B);
...
function StreetPicker(_ref5) {
  ...
  var q = _useState202[0];
  var setQ = _useState202[1];

  var _useState21 = useState(false);

  var _useState212 = _slicedToArray(_useState21, 2);

  var open = _useState212[0];
  var setOpen = _useState212[1];
  ...
  return React.createElement(
    ...
    React.createElement('input', { dir: 'rtl', value: q, placeholder: icon + ' ' + label + ' — הקלד שם רחוב…', onFocus: function () {
        return setOpen(true);
      }, onChange: function (e) {
        setQ(e.target.value);setOpen(true);
      }, style: selStyle }),
    open && matches.length > 0 && React.createElement(
      'div',
      { style: { position: 'absolute', zIndex: 1000, top: '100%', insetInlineStart: 0, insetInlineEnd: 0, background: '#0f2530', border: '1px solid #2c4a56', borderRadius: 8, maxHeight: 240, overflowY: 'auto', boxShadow: '0 8px 24px rgba(0,0,0,.5)' } },
      matches.map(function (m, i) {
        return React.createElement(
          'div',
          { key: i, onMouseDown: function () {
              onPick(m);setQ(m.name);setOpen(false);
