...
var CHANGES_URL = 'data/junction_changes.json?v=2026-07-27g';
...
function JunctionApp() {
  ...
  useEffect(function () {
    ...
    try {
      for (var _iterator20 = finalEff.features[Symbol.iterator](), _step20; !(_iteratorNormalCompletion20 = (_step20 = _iterator20.next()).done); _iteratorNormalCompletion20 = true) {
        ...
        if (f.kind === 'marked' && f.style === 'lrt_only') drawNewRoad(f.segs, f.style, f.label);else if (f.kind === 'closed') L.polyline(toLL(f.coords), { color: '#e5484d', weight: 6, opacity: .85, dashArray: '4 8' }).addTo(layer).bindTooltip(f.label || 'מקטע סגור', { sticky: true });
      }
    } catch (err) {
  ...
  useEffect(function () {
    ...
    var roads = []; // new-road polylines {cas, fil} — width scales with zoom (metric), OSM-like
    var roadW = function roadW() {
      // pixel widths from a ~5 m single carriageway at current zoom
      var mpp = 156543.03392 * Math.cos(31.758 * Math.PI / 180) / Math.pow(2, map.getZoom());
      var fill = Math.max(1.6, Math.min(34, 5 / mpp));
      return { fill: fill, cas: fill + Math.max(1.1, fill * 0.28) }; // thin casing, like the basemap
    };
          ...
          try {
            for (var _iterator23 = lines[Symbol.iterator](), _step23; !(_iteratorNormalCompletion23 = (_step23 = _iterator23.next()).done); _iteratorNormalCompletion23 = true) {
              ...
              if (kind === 'new_axis') {
                ...
                var cas = L.polyline(ll, { color: '#d4cebf', weight: w.cas, opacity: .9, lineCap: 'round', lineJoin: 'round' }).addTo(layer);
                var fil = L.polyline(ll, { color: '#ffffff', weight: w.fill, opacity: 1, lineCap: 'round', lineJoin: 'round' }).addTo(layer);
                roads.push({ cas: cas, fil: fil });
    ...
    var onZoom = function onZoom() {
      var w = roadW();var _iteratorNormalCompletion24 = true;
      ...
      try {
        for (var _iterator24 = roads[Symbol.iterator](), _step24; !(_iteratorNormalCompletion24 = (_step24 = _iterator24.next()).done); _iteratorNormalCompletion24 = true) {
          var r = _step24.value;
          r.cas.setStyle({ weight: w.cas });r.fil.setStyle({ weight: w.fill });
        }
      } catch (err) {
    ...
    if (cadRef.current) draw(cadRef.current);else fetch('data/junction_cad.geojson?v=2026-07-28a').then(function (r) {
      return r.json();
    }).then(function (fc) {
